...
/*
每个函数实际都只是一个wrapper, 实际都是调用的$wp_query对象的方法
*/ 
...
使用query_posts(), 因为查询结果放到全局变量中了，因此依然可用the_title(), ....这种简洁tag .
使用$a = get_posts(), 则得$a[0]->get_the_title() 这么用, 但在模板中使用tag不方便(要通过使用setup_post后才能用tag)
都不含当前页面url中的参数

 */ 
function query_posts($query) {
	$GLOBALS['wp_query'] = new WP_Query();
	/* $query是不包括页面url中的参数的, 如果要用url中的参数就得先parse后merge到$query中 */
...
function wp_reset_query() {
	/* 平时用的是wp_query, wp_the_query是原始的, 现在wp_query恢复成原始的 */
	$GLOBALS['wp_query'] = $GLOBALS['wp_the_query'];

	/* 把原始query中的post也转到全局当前post中 */
	wp_reset_postdata();
}
...
class WP_Query {
	 ...
	 *     @type int          $p       贴子ID                Post ID.
	 *     @type int          $page                    Show the number of posts that would show up on page X of a
	 *                                                 static front page.
	 *     @type int          $paged       当前页号            The number of the current page.
	 ...
	 *     @type string       $s     搜索关键词                  Search keyword(s). Prepending a term with a hyphen will
	 ...
	 */
	 /*
	分析请求页面的$_GET参数, 设置一些请求标志, 如is_single, is_page,...
	s: search keyword
	p: post id
	name: post slug // 如果查数是一个page(不是post)的slug, 会不会查到呢?
	
	page_id: page id
	pagename: page slug
	
	cat: 分类id, 可为数组如cat=3,4,5  cat=-5(负号表示排除)
	category_name: category的slug
	...
	public function parse_query( $query =  '' ) {
		...
		*/		
		...
		if ( ('' != $qv['attachment']) || !empty($qv['attachment_id']) ) {
			...
			// 如$_GET中有'p'参数, 即要某个post id, 意味着请求也是single
			$this->is_single = true;
		} elseif ( ('' !== $qv['hour']) && ('' !== $qv['minute']) &&('' !== $qv['second']) && ('' != $qv['year']) && ('' != $qv['monthnum']) && ('' != $qv['day']) ) {
			// If year, month, day, hour, minute, and second are set, a single
			// post is being queried.
			// 年月日时分秒参数都有, 也表示是single
			$this->is_single = true;
		} elseif ( '' != $qv['static'] || '' != $qv['pagename'] || !empty($qv['page_id']) ) {
			// 如$_GET中有'page_id'参数, 表示请求是page, 
			$this->is_page = true;

			// 不是单个???, 指定了page_id应当是单个吧?
			$this->is_single = false;
		} else {
			...
			/* 检查是否有分类方面的参数, 生成$this->tax_query这个WP_Tax_Query对象 */
	 ...
	 */
	 /* 
	 检查参数中是否有自定义的分类 ,cat, tag这样的参数
	 查询参数整理后,生成一个WP_Tax_Query对象, 放到$this->tax_query
	 */
	public function parse_tax_query( &$q ) {
		/* 与date_query, meta_query 一样,以支持tax_query 用法*/
	...
	public function get_posts() {
		...
		// Parse meta query
		/*** url中有些参数被分到WP_Meta_Query中, 有些被分到WP_Tax_Query,  它们产生的sql会被合到一个大sql中 */
		...
		if ( $date_parameters ) {
			/* 将所有日期型参数组成一个WP_Date_Query对象 */
			$date_query = new WP_Date_Query( array( $date_parameters ) );
			/* 得到这个WP_Date_Query对象 的SQL */
		...
		if ( !$this->is_singular ) {
			 /* 
			 检查参数中是否有自定义的分类 ,cat(如1,2,3), category_name(category slug), tag这样的参数
			 查询参数整理后,生成一个WP_Tax_Query对象, 放到$this->tax_query
			 */		
			$this->parse_tax_query( $q );

			/* 由$this->tax_query这个WP_Tax_Query对象得到SQL条件 */
		...
		if ( $split_the_query ) {
			...
			/*** 一个大的联合查询, 有时可以分成二次查询. 先得到id集合, 再根据每个id查详情 */
		...
		/* 检查一遍posts数组, 保证里面每个元素是WP_Post对象 */
		if ( $this->posts ) {
			$this->post_count = count( $this->posts );
	...
	public function the_post() {
		...
		if ( $this->current_post == -1 ) // loop has just started
			 ...
			 * @param WP_Query &$this The WP_Query instance (passed by reference).
			 */
			 // 循环之前, 先执行挂在 'loop_start'点上的所有函数
			do_action_ref_array( 'loop_start', array( &$this ) );

		$post = $this->next_post();

		/* 
		这个贴子内容是否分页, 作者信息, 写作时间,等, 将这些信息放到全局变量中,当然贴子
		的有些信息还是得到全局$post中看, 如$post->post_content, 不过这样取内容太原始, 如果内容中有
		[gallery]这样的shortcode,得不到解释, 所以最好用模板tag取数据
		
		模板除了能用函数tag外,还能使用全局变量tag, 它包括url中的参数和读到的post中的字段 
		也就是说, 在模板文件中可直接使用$p, $paged, $
		这post中的内容卸出到全局, 这样模板文件中可以直接使用全局变量tag 
		*/
	 ...
	 */
	 /* 
	 查询时传入的参数数组, 传入后与default参数合并后做select查询 
	 WP_Query对象并不与url串直接发生关系, 它只与传入的参数发生关系
	 */
	 ...
	 * @return bool Whether the query is for an existing single post of any of the given post types.
	 */
	 /*** 等价于is_single() || is_page() || is_attachment()  ? */
	 ...
	 */
	 /* 
	 设置贴子的标志: more, $multipage 
	 将$post这个WP_Post对象中的数据全部卸到全局变量中? 这样模板中就可以用全局变量$tag, 错了.
	 */
...
setup_postdata( $myPost );       // 这样之后, 模板中才能使用the_title(), the_content()显示$myPost的东西
the_title();
the_content();
 */
