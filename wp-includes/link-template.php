...
function get_the_posts_pagination( $args = array() ) {
	...
	if ( $GLOBALS['wp_query']->max_num_pages > 1 ) {
		$args = wp_parse_args( $args, array(
			...
			/*** 
			screen-reader-text使字很小,正常人看不到,但盲人借助设备可以理解,如下
 ...
 */
 /***
$args = get_avatar_data(1) 
$args = get_avatar_data('abc@md5.gravatar.com')   // 通过@md5.gravatar.com直接指定abc这个hashed email
$args = get_avatar_data('demo@sina.com')		// 把这个email md5一下, 得到hashed email
 */
function get_avatar_data( $id_or_email, $args = null ) {
	...
	/*** 想办法找出用户的hash email, 可以是直接取到的,也可以是用md5(user_email)取到的 */
	if ( ! $email_hash ) {
		if ( $user ) {
