 ...
 */
 /*
在loop内可以直接$a = get_the_title(), 在loop外得带参数$a = get_the_title($my_post_id)
 */
 ...
 */
 /*
guid指全局唯一id, 
wordpress使用了get_permalink($post_id) 作为唯一id, 看起来这个guid像链接(http://127.0.0.1/note-wordpress/?p=1), 但是不应当作链接使用?
guid的作用?
 */
...
取当前贴子的当前页的内容
在非单页面如home页, search页中会显示teaser(即more tag之前的内容)和more链接
在单页面中如某贴子, 则显示全部内容

$more_link_text: '阅读更多',可以设成是'read more>>'', 此参数也可以tag中设置
teaser: 前导, 预告,指more tag之前的内容
$strip_teaser: 是否不显示more前面的内容即teaser

有时一篇贴子很长, 经常先写个头子, 加more tag, 再加其它内容.
在显示列表时就会只显示头子,点more链接,才会显示全部

写贴子时, 2种情况:
1. 先写贴子的一部分, 这时加more tag, 这里more tag的含义就是想看更多就点一下
2. 利用more tag实现摘要机制, 即先写一小段, 加more tag, 再写全部贴子内容. 在用户看单页时,如果不想看到摘要,可加<!-- noteaser --> tag
...
function get_the_content( $more_link_text = null, $strip_teaser = false ) {
	...
	if ( preg_match( '/<!--more(.*?)?-->/', $content, $matches ) ) {
		...
		if ( ! empty( $matches[1] ) && ! empty( $more_link_text ) )
			/* $more_link_text的内容还可以在tag中, 指定<!-- moreABC --> 会取'ABC'替换'read more>>' */
 ...
 */
 /*** 
 显示摘要, 如果摘要为空就取内容中前55个字
 
 more tag与excerpt是2个不同的东西,但作用比较类似,http://wpninjas.com/post-excerpt-read-more-tag/
 区别在于
 excerpt能更精确的控制摘要内容, 但在模板文件中要加the_excerpt的标签
 而more标签直接在贴子内容中, 由系统自动识别, 不需要the_excerpt标签, 只要the_content标签就行了,换主题时也没问题
 所以如果对摘要要求不高的话, 使用more更好
 */ 
...
function get_the_excerpt( $post = null ) {
	 ...
	 */
	 /***  wp_trim_excerpt() 默认会挂在'get_the_excerpt'上 */
...
function get_body_class( $class = '' ) {
	...
	/*** custom background 其实就是通过一个css类发挥作用 */
...
返回一列
<select name="myname">
<option value=page_id>title1</option>
...
<option value=page_id>title2</option>
</select>
*/ 
 ...
 */
 /* 
以树形的形式显示一堆post_type为page的, 使用Walker_Page
 */
