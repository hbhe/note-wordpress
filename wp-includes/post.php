...
一篇文章(post_type), 它可能属于外部新闻与内部新闻
但是对于产品这个post_type, 显示它与上面2种分类无关,
这个时候就是新建一种taxnomy了, 比如叫产品分类
从界面上看, taxnomy是放在post type 菜单下, 因此建一种post_type，就应当建相应的taxonomy.

存在表wp_posts中的post记录分为哪几种类型, 哪几种状态,这种信息保存在内存中,而不是在db中, 
...
function create_initial_post_types() {
	...
	是由register_taxonomy()决定的,此函数表明一个taxnomy对应哪几个post_type
	taxonomy与post_type是个多对多的关系
	*/
	register_post_type( 'post', array(
		...
		/*** 静态指定post这种post_type支持的features, 以后还可以通过add_post_type_support() 动态增减 
		feature用来控制贴子内容的输入, 比如
		如果拿掉'title', 建新贴子时就不会有title这个栏目!
		如果拿掉'editor', 就不会出现编辑正文的富文本编辑框
		
		可供post_type直接使用的内建features有11种,可理解成分别对应wp_posts表的11个字段(其实有些是meta字段)!
		'title'		控制title
		'editor'     控制正文content
		'author'
		...
		'page-attributes' (template and menu order) (hierarchical must be true, 必须是有层次关系的post_type才行?)
 ...
 */
 ...
 /*
 转换post的格式, 默认是将数组转成WP_Post对象
$a = get_post( 123, ARRAY_A ); 返回一个array, 而不是一个WP_Post对象
 */
...
function register_post_type( $post_type, $args = array() ) {
	...
	/*** 
	将某post_type默认生成的权限名与传入的权限名合并,
	即将操控此post_type时需要的权限名, 都放在$args->cap中,  这样的话以后通过
	current_user_can( $post_type_object->cap->edit_posts )    //是否有编辑所有此post_type的权限 
	$post_type_object->cap->edit_posts可能对应于edit_pages, edit_posts, edit_products, ...
	用current_user_can( $post_type_object->cap->edit_posts ) 而不是用current_user_can('edit_products');
	*/
 ...
 */
 /***
根据post_type, 返回一组操作此post_type时需要的权限名字(单数读写删, 复数读写删), 如
read_product, edit_product, delete_product,
read_products, edit_products, delete_products

capability_type包括就是单数和复数单词
 */
 ...
 */
 /***
 theme可以有feature, 不同的post_type也可以有不同的features
 某个post_type会有多个features, create_initial_post_types()指定了常用的几种post_type的features :
 ...
 */
 /**
get_posts()与query_posts()的区别? 
get_posts()主要是后台用, 一次性得到结果
query_posts()主要是在模板文件中用
 ...
 */
 /***
操作post meta数据,可通过
1. add_post_meta(),    其实这个也是通过调用通用的add_metadata()实现的
2. add_metadata(),   这个可操作所有meta表的数据
 */
...
_thumbnail_id			// 当为贴子指定一个缩略图时, 会在wp_postmeta表中加一条记录(如meta_key:_thumbnail_id, meta_value:100  )
_wp_page_template    // 为本贴子指定的page template
_edit_lock                 // 每个贴子都有这2个meta key, 既如此,为何不就放在wp_posts表中
_edit_last
...
function wp_insert_post( $postarr, $wp_error = false ) {
	...
	if ( 'attachment' === $postarr['post_type'] ) {
		/*** 如果插入的是图片类, 文件名和图片说明就放到postmeta数据中 */
		if ( ! empty( $postarr['file'] ) ) {
			update_attached_file( $post_ID, $postarr['file'] ); 		// 对应_wp_attachment_file
		}
 ...
 */
 /* 
一个贴子内可能有多个指向外网的链接, 每向外网的一个链接发送pingback成功之后 , 
都要将这个链接添加到贴子的pinged字段中
pinged字段反映的是某贴子发送pingback的情况
*/
 ...
 */
 /*
取出一堆页面page(也是WP_Post类型)
与get_posts()类似, 为何要单独搞这么个函数
 */
...
function wp_get_attachment_metadata( $post_id = 0, $unfiltered = false ) {
	...
	/***
	error_log(print_r(get_post( $post_id , ARRAY_A) , true)); 		
	wp_post表中attachment类型的记录为
	(
	    [ID] => 181
	    [post_author] => 2
	    ...
	    [post_content_filtered] => 
	    [post_parent] => 202
	    [guid] => http://127.0.0.1/note-wordpress/wp-content/uploads/2013/06/cd_6_angle.jpg
	    [menu_order] => 0
	...
	其_wp_attachment_metadata meta字段的值为:
	error_log(print_r($data, true)); 	
	[06-Aug-2016 05:12:27 UTC] Array
	(
	    [width] => 1000
	    [height] => 1000
	    [file] => 2013/06/cd_6_angle.jpg
	    [sizes] => Array
	        (
	            [thumbnail] => Array
	                (
	                    [file] => cd_6_angle-150x150.jpg
	                    [width] => 150
	                    [height] => 150
 ...
 */
 /*
 编辑贴子之后要求以后在合适的时候发pingback
 发布一个贴子之后, 还要做一些事情包括:立即执行的、以后要执行的计划
 */
function _publish_post_hook( $post_id ) {
	...
	/* 如果可向它人发pingback, 此标志在后台管理内设置 */
	if ( get_option('default_pingback_flag') )
		add_post_meta( $post_id, '_pingme', '1' );  /* 以后会检查所有_pingme为1的贴子,以向他人发pingback, 以后发一个pingback,删除一个_pingme */
		
	add_post_meta( $post_id, '_encloseme', '1' );

	/* 
	存一个计划到db中, 这个计划的内容是:当时间超过time()时,执行挂在do_pings上的函数,如do_all_pings() 
	注意参数'do_pings'是一个hook锚点, 并不是callback(在hook锚点上才挂着一些callback)
	*/
	// 发完贴子并不是立即执行do_all_pings()
