 ...
 */
 ...
 /*** 
 在原url中加一个参数后(如果此参数在原url存在, 就覆盖它), 返回新的url
 add_query_arg( 'key', '5'); 返回 当前url参数后加key=5      
 add_query_arg( 'key', false); 当前url参数中去掉key=5参数串
 add_query_arg( 'key', '5', 'http://example.com/' ); 返回 http://example.com/?key=5      
 add_query_arg( 'key', '5', 'http://example.com/?key=1' ); 返回 http://example.com/?key=5       
...
function do_feed() {
	...
	/* 
	http://127.0.0.1/note-wordpress/?feed=rss2 表示订阅该网站的贴子,
	...
	*/
	 ...
	 */
	 /* 执行do_feed_rss2(), 或do_feed_comments_rss2() */
 ...
 */
 /***  
 文件上传到哪个目录下, 默认是uploads/yyyy/mm目录下 ,
 本函数是无过滤, 无cache的实现, wp_upload_dir()是它的一个wrapper
 */
...
function wp_die( $message = '', $title = '', $args = array() ) {
	...
	if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
		 ...
		 * @param callable $function Callback function name.
		 */
		 /* 默认执行_default_wp_die_handler()函数 */
 ...
 */
 /* 将$defaults 导入$args中, 或者说将$args中的参数补齐 */
