 ...
 */
 /*
在$tag点埋下将来要执行的函数
 埋点时,除了提供函数名$function_to_add外, 还要提供函数的参数个数$accepted_args
 $priority小的先执行

 add_filter(), add_action()都是共用一个全局变量$wp_filter, 所以filter和action的tag不能相同
 */
 ...
 */
 /*
依次执行挂在$tag这个锚点上的所有函数, 
前一个函数的返回结果作为下一个函数的传入参数

本来可以直接$a = 840; 但$a = apply_filters( 'twentysixteen_content_width', 840 );
给了外面一个修正的机会, 如果有挂在'twentysixteen_content_width'上的函数, 取函数返回值, 否则就取840
 */
