...
function wpdocs_theme_name_scripts() {
	...
	wp_enqueue_script( 'myjs', get_template_directory_uri() . '/js/example.js', array(), '1.0.0', true );
...
add_action( 'wp_enqueue_scripts', 'wpdocs_theme_name_scripts' );		// wp_enqueue_scripts这个名字同时适用于js,css

错误的用法,虽然简单,但是没有充分利用依赖管理, 比如如果多个插件都这么加载jquery, 就会导致重复加载
...
function _wp_scripts_maybe_doing_it_wrong( $function ) {
	/*** 一定要先init, 后wp_enqueue_scripts(), 否则报错*/
	if ( did_action( 'init' ) ) {
		return;
	}
 ...
 */
 /***
 如何做到, 在指定的页面中指定的位置加载script?
 
$in_footer:是插在header位置还是在footer位置

function wpdocs_theme_name_scripts() {
    wp_enqueue_style( 'mycss', get_stylesheet_uri() );
    wp_enqueue_script( 'myjs', get_template_directory_uri() . '/js/example.js', array(), '1.0.0', true );
}
add_action( 'wp_enqueue_scripts', 'wpdocs_theme_name_scripts' ); // 前台页面上加载js,css
add_action( 'admin_enqueue_scripts', 'xxx' ); // xxx脚本放在管理页面
add_action( 'login_enqueue_scripts', 'xxx' ); // xxx放在login页
...
function wp_enqueue_script( $handle, $src = false, $deps = array(), $ver = false, $in_footer = false ) {
	...
	/***
	先init, 后wp_enqueue_script()
	在模板文件中用wp_enqueue_script()可能已经晚了吗? 此函数一般在functions.php中被使用?
	但如果我希望只是在某类页面甚至是某个页面上加载, 在functions.php阶段如何判断?
	if ( is_singular() ) {
		wp_enqueue_script()
	}
	*/
