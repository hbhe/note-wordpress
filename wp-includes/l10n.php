 ...
 */
 /*
在英语中, 名词有单复数之分
我有1条新消息, I have 1 new message , 用printf( __( 'You have %d new message', 'prowp-plugin' ), $count );可表示
我有2条新消息: I have 2 new messages,  用printf( __( 'You have %d new messages', 'prowp-plugin' ), $count );可表示
 ...
 */
 /* 
 加载一个翻译文件,如
 load_textdomain( 'project1', WP_LANG_DIR . "/languages/theme/mytheme.mo" );
 以后就可以用project1这个词典了,如
 _e('hi, this is my word','project1')
 系统中通常有很多本词典,如'default', 'plugin1', 'theme1', ... , 
 $domain就是指明哪一本词典, 有点像php.ini中的[section], 组的意思,用于对一个超大词典分组
 */
...
function load_default_textdomain( $locale = null ) {
	...
	/* 加载wp-content/languages/zh_CN.mo */
...
function load_plugin_textdomain( $domain, $deprecated = false, $plugin_rel_path = false ) {
	...
	/* 先查当前插件目录下wp-super-cache-zh_CN.mo, 如wp-content\plugins\wp-super-cache\languages\wp-super-cache-zh_CN.mo */
	$mofile = $domain . '-' . $locale . '.mo';
	if ( $loaded = load_textdomain( $domain, $path . '/'. $mofile ) )
		return $loaded;

	// Otherwise, load from the languages directory
	/* 再找wp-content/languages/plugins/wp-super-cache-zh_CN.mo */
...
function load_theme_textdomain( $domain, $path = false ) {
	...
	/* 优先到当前模板目录下找mo文件,zh_CN.mo, 如wp-content/themes/wpdx/languages/zh_CN.mo */
	$mofile = untrailingslashit( $path ) . "/{$locale}.mo";
	if ( $loaded = load_textdomain( $domain, $mofile ) )
		return $loaded;

	// Otherwise, load from the languages directory
	/* 再到wp-content/languages/目录下找themes/twentysixteen-zh_CN.mo, 如D:\htdocs\note-wordpress/wp-content/languages/themes/twentysixteen-zh_CN.mo */
