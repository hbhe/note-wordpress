...
每一张图片与post一样,在wp_posts中都有一条记录(post_type为attachment),并且
在wp_postmeta表中有2个meta_key
_wp_attached_file  		// 记录图片原始文件
_wp_attachment_metadata	// 记录各种size的图片信息(上传图片时, 会生成各种size的图片)

wp_get_attachment_image(290, 'thumbnail');    // 产生thumbnail号的图片html
wp_get_attachment_image(290, 'medium');		// 产生medium号的图片html
...
 ...
 */
 /***
1. add_image_size( 'wechatpress_sm', 80, 80 );
2. 上传文件时,会在wp_posts中插入一条记录,如id为293
3. 利用id就可以访问图片, wp_get_attachment_image(293, 'wechatpress_sm')就会显示刚才上传的80x80的图片
    如果80x80的图片不存在, 会不会重新自动生成? 
    不应当是在上传时缩放图片吧? 应当是在使用的时候才生成更好些吧?
 */
 ...
 */
 /*** 返回
<img width="1000" height="1000" src="http://127.0.0.1/note-wordpress/wp-content/uploads/2013/06/cd_6_angle.jpg" 
	class="attachment-post-thumbnail size-post-thumbnail wp-post-image" alt="cd_6_angle" 
srcset="http://127.0.0.1/note-wordpress/wp-content/uploads/2013/06/cd_6_angle.jpg 1000w, 
	http://127.0.0.1/note-wordpress/wp-content/uploads/2013/06/cd_6_angle-150x150.jpg 150w, 
	http://127.0.0.1/note-wordpress/wp-content/uploads/2013/06/cd_6_angle-300x300.jpg 300w, 
	http://127.0.0.1/note-wordpress/wp-content/uploads/2013/06/cd_6_angle-768x768.jpg 768w, 
	http://127.0.0.1/note-wordpress/wp-content/uploads/2013/06/cd_6_angle-180x180.jpg 180w, 
	http://127.0.0.1/note-wordpress/wp-content/uploads/2013/06/cd_6_angle-600x600.jpg 600w" 
sizes="(max-width: 709px) 85vw, (max-width: 909px) 67vw, (max-width: 984px) 60vw, (max-width: 1362px) 62vw, 840px" />
srcset,sizes属性用于图片自适应


每一张图片与post一样,在wp_posts中都有一条记录,并且
在wp_postmeta表中有2个meta_key
_wp_attached_file  		// 记录图片原始文件
_wp_attachment_metadata	// 记录各种size的图片信息
...
function wp_get_attachment_image($attachment_id, $size = 'thumbnail', $icon = false, $attr = '') {
	...
	Array
	(
	    [0] => http://127.0.0.1/note-wordpress/wp-content/uploads/2013/06/cd_6_angle.jpg
	    [1] => 1000
	    [2] => 1000
	    [3] => 
	)
...
function wp_max_upload_size() {
	// 取php.ini中的upload_max_filesize, post_max_size
	 ...
	 */
	 /* 取upload_max_filesize, post_max_size中较小的值作为最大上传文件大小  */
