...
class WP_Image_Editor_GD extends WP_Image_Editor {
	 ...
	 */
	 /***
	 缩放图片, 也就是修改当前对象的image, size属性, 一般与save()一起配合使用
	 $this->resize(100, 100);
	 $this->save();			// 不带文件名的话, 就取原文件名+ 宽x高.jpg 作为文件名
	 如何使用@see image_make_intermediate_size()
	 */
	...
	protected function _resize( $max_w, $max_h, $crop = false ) {
		...
		/*** 新建image资源, 将原图上的矩形区域copy到新建的image上 */
	 ...
	 */
	 /*** 
	 对当前图片resize成各种size的文件并保存, 返回各次resize后的情况
	 */
	public function multi_resize( $sizes ) {
		...
		foreach ( $sizes as $size => $size_data ) {
			...
			if ( ! is_wp_error( $image ) && ! $duplicate ) {
				/*** 将缩放后的文件导出, 新文件名后加-150x150.jpg之类 */
	...
	protected function _save( $image, $filename = null, $mime_type = null ) {
		...
		if ( ! $filename )
			/***  产生新文件名形如...-150x150.jpg,  WordCamp-San-Francisco-150x150.jpg */
