...
/*
object caching system是option操作要依赖的组件
*/
 ...
 */
 /*
 先从cache中取,如果没有再从db中取,默认的cache使用的是wordpress自带的单页面caching, 不能在多页面中共享,
 也就是说A页面中调用get_option('name')时,在A页面中别的位置再次调用get_option('name')时会从cache中取,但在B页面中
 调用get_option('name')时会重新从db中取
 ...
 */
/*
从cache中得到一个跨页面存在的value
如果未提供apc caching(在object-cache.php中实现的), 就利用get_option() 从db中取
否则,直接从apc中取

当有了wp-content/object-cache.php文件get_transient()与wp_cache_get()等价
否则get_transient()是跨页面cache, wp_cache_get()是单页面caching
*/ 
 ...
 */
 /*
 wordpress中默认是用option表做缓存, 比如想缓存'my_score'='100' 超时时间为60秒,那么
 在option表中加2行记录
_site_transient_my_score, 100
_site_transient_timeout_my_score, time()+60
 */
// 得到临时变量的值 
