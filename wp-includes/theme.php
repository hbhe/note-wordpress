 ...
 **/
 /* 
 对于子主题,  TEMPLATEPATH与STYLESHEETPATH不相等, 
 TEMPLATEPATH为父主题所在目录, STYLESHEETPATH为子主题所在目录
 */
...
function get_template_directory() {
	...
	// $template_dir 形如D:\htdocs\note-wordpress/wp-content/themes/twentysixteen
	/**
 ...
 * @return bool
 */
 /* 检查当前主题目录下是否有index.php和style.css文件, 否则切换到WP_DEFAULT_THEME主题,
 如果WP_DEFAULT_THEME也不存在, 再从历史默认主题中找一个 
 后台点外观->主题(或自定义)时会到这里
 */
function validate_current_theme() {
	...
	if ( ! file_exists( get_template_directory() . '/index.php' ) ) {
		...
		// 子主题可以没有任何php文件, 但至少必须有一个style.css
		// Invalid.
	} else {
		// Valid.
		return true;
	 ...
	 */
	 /* 如果WP_DEFAULT_THEME也不存在, 再逐个尝试2016, 2015, 2014, ...等默认主题 , 取一个*/	 
...
在$_wp_theme_features中注册$feature, 保存的是这个feature需要的参数数组, 
...
如何使用这些feature, 参见edit-form-advanced.php
 */
function add_theme_support( $feature ) {
	...
	switch ( $feature ) {
		/*** 像'post-formats'这样的feature, 如果注册时不传入参数, 就会取default值 */
		case 'post-formats' :
			if ( is_array( $args[0] ) ) {
...
function _wp_customize_include() {
	...
	/* 如果是后台管理的customize.php页面,则加载主题定制管理器 */
