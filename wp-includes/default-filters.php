...
/*** 
在显示html 的head时, 才开始收集用到的css, js, 只是收集而已,还不是输出 
*/
...
/*** 
这里才是真正输出(print)前面收集到的css, 可 见挂在wp_print_styles和wp_enqueue_scripts都是可以的,
不过最好放在wp_enqueue_scripts()中, 放在wp_print_styles的话因们它直接输出, 不会处理继承关系,所以会有重复加载问题?
*/
add_action( 'wp_head',             'wp_print_styles',                  8    );

/*** 这里才是真正输出(print)前面收集到的js(位置头部) */
...
/* 每更新一次贴子, 在post中插入一条版本记录? */
add_action( 'post_updated',               'wp_save_post_revision',                   10, 1 );

/* 发布贴子后, 做一些事, 包括:计划发pingback给贴子中曾引用到的外部主机? */
...
// REST API actions.
/* 
http://127.0.0.1/note-wordpress/?rest_route=/ 可列出所有支持的服务
API 接口 , 如何使用rest api?
比如我提供写了一个函数product_list(), 想对外服务, 得用如register_route('/products', 'product_list')注册之后才行,
可仿照class-wp-oembed-controller.php写
可使用某些插件, 它把post, comment的api都写好了
*/
add_action( 'init',          'rest_api_init' );
/* new 一个server之后, 就会fire它, 一般在此锚点上挂些register_route() */
add_action( 'rest_api_init', 'rest_api_default_filters', 10, 1 );
/* 每个请求都要检查是否url中带有rest_route参数,如有直接执行前面注册过的函数, 给出response后退出 */
...
// Embeds
/* 
注册一组rest 路由, 向外网提供对Embeds的操作(new一个WP_oEmbed_Controller, 并通过register_route()注册)
WP_oEmbed_Controller是一个标准rest 实现,比如想对外公开product, order 的服务, 可参考之
*/
