...
function do_all_pings() {
	...
	// Do pingbacks
	/* pingback是自动搜索贴子内容,发现有指向外网的链接就发pingback */
	while ($ping = $wpdb->get_row("SELECT ID, post_content, meta_id FROM {$wpdb->posts}, {$wpdb->postmeta} WHERE {$wpdb->posts}.ID = {$wpdb->postmeta}.post_id AND {$wpdb->postmeta}.meta_key = '_pingme' LIMIT 1")) {
		/* 删一条_pingme记录,发一个pingback */
	...
	// Do Trackbacks
	/* 
	trackback是手工式的pingback, 即搜索哪些待发pingback的贴子(即to_ping不为空的), 但是这个to_ping标志得人工加上去	
	*/
	$trackbacks = $wpdb->get_col("SELECT ID FROM $wpdb->posts WHERE to_ping <> '' AND post_status = 'publish'");
	if ( is_array($trackbacks) )
		foreach ( $trackbacks as $trackback )
			/* 贴子的to_ping字段包含多个外网地址, 逐个向它们外trackback包, 发一个to_ping中少一个地址,同时pinged中多一个 */
	...
	if ( $to_ping ) {
		foreach ( (array) $to_ping as $tb_ping ) {
			...
			if ( !in_array($tb_ping, $pinged) ) {
				/* 向外网url发trackpack包, 发成功一个, pinged串加上url, 同时to_ping串减去此url  */ 
				trackback($tb_ping, $post_title, $excerpt, $post_id);
				$pinged[] = $tb_ping;
			} else {
				/* 如果曾向此外网url发过pingback, 从to_ping串中减去此url就好了 */
...
posts表中有3个标志 ping_status, to_ping, pinged, comment_status
ping_status: 'open'表示在保存贴子时允许系统向他人发pingback, (不是允许被外网ping, 这个也禁止不了)
to_ping: 待向其发trackback的外网url, 以后发成功一个url, to_ping就减一个url, 同时pinged加一个url
pinged:  字符串, 已经向外网发pingback成功过的链接, 多个链接以\n分隔
comment_status : 'open'表示此贴允许被他人评论
 
在贴子内容中找出所有引用外网贴子的链接, 
逐个向这些外网链接以POST 向它们发XML pingback包, 如成功再把外网地址加到贴子的pinged 字段
 */
function pingback($content, $post_ID) {
	...
	foreach ( (array) $post_links as $pagelinkedto ) {
		...
		if ( $pingback_server_url ) {
			...
			/* 以HTTP POST的形式向外网提交pingback XML包*/
			if ( $client->query('pingback.ping', $pagelinkedfrom, $pagelinkedto) || ( isset($client->error->code) && 48 == $client->error->code ) ) // Already registered
				/* 如成功提交,  将外网地址记录到贴子的pinged字段 */
 ...
 */
 /* 
 向外网发trackpack包, 发成功一个, pinged串加上url, 同时to_ping串减去此url 
 */
