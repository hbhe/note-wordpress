...
/* 如果有自己的脚本要在安装时执行,存在install.php中 */
...
function wp_install( $blog_title, $user_name, $user_email, $public, $deprecated = '', $user_password = '', $language = '' ) {
	...
	/*** 将系统所支持的所有角色存到wp_options表中, 即更新wp_options中的wp_user_roles这个key */
	populate_roles();
 ...
 */
 /*
按sql语句(或者'all', 'blog'这样的缩写),  调整实际存在的db表结构

如果要求建表, 但以前没有表, 就创建
如果要求建表, 但以前有表,只是表结构有变化, 就更改表结构(只加字段,不减字段)
如果要求建表, 但以前有表, 且表结构无变化, 什么都不做
如果是INSERT, UPDATE直接执行就好了, 不支持ALTER TABLE, DELETE FROM这种
 */
function dbDelta( $queries = '', $execute = true ) {
	...
	/*** 可见dbDelta() 只支持CREATE TABLE, INSERT, UPDATE  */
	...
	foreach ( $cqueries as $table => $qry ) {
		...
		/*** 
		新表中的所有字段放到$cfields ,
		对于那些字段是在老表中已有的, 且字段类型也没变的, 把它们从$cfields拿掉   
		对于那些字段是在老表中已有的, 但字段类型有变的, 把它们从$cfields拿掉, 同时执行ALTER TABLE  CHANGE COLUMN                 
		$cfields最后就是纯要增加的字段了
		*/
		foreach ($flds as $fld) {
			...
			$validfield = true;
			/*** 如果字段名为primary, index, unique, key则认为是索引, 可见不支持如下写法(MYSQL是认识的)
			id int(10) unsigned NOT NULL AUTO_INCREMENT  PRIMARY KEY,    // PRIMARY KEY放在后面, 不识别
			KEY idx_EventKeyCRC(EventKeyCRC)    // idx_EventKeyCRC (EventKeyCRC)   要用空格分开?
			*/
		...
		Array ( 
			[0] => PRIMARY KEY (id) 
			[1] => KEY idx_gh_id_scene_pid(gh_id,scene_pid) 
			[2] => UNIQUE KEY idx_gh_id_open_id(openid) 
		)   
		*/
		...
		foreach ($tablefields as $tablefield) {
			...
			/* 旧表(实际现存表)中的字段也存在于schema中, 进一步检查该字段的字段类型是否发生了变化 */
			if (array_key_exists(strtolower($tablefield->Field), $cfields)) {
				...
				/*
				实际表中有些字段,在新的schema中不存在
				此时是否可以删掉这些字段?
				不可以!
				用户可以扩展表中的字段(不过字段名要自己保证唯一性),wordpress在升级时不会删掉那些系统不识别的字段
				*/
			}
		}
