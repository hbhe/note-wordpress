 ...
 */
 /* 
一个post的除了常规数据外,还有所谓的meta数据,因此除了编辑title,content外,
还要能编辑meta数据,meta-box相当于是一个可编辑meta的widget(也不尽然, 在输入post_format这个非meta数据时也是用的add_meta_box())
add_meta_box()是准备以后要输出的meta数据
do_meta_boxes()才是输出meta框的html片断 

$screen表示在哪个(或哪类)页面才显示, 如'post','page'这种post_type, 
为什么是screen, 而不是post_type?  如果使用时不指定$screen这个参数，
比如我只想为编辑post时加个metabox, 就用$screen = 'post'参数
 ...
 */
 /*
add_settings_section()准备,对某设置再加一个section, do_settings_sections()输出
$callback函数返回的是将来要echo的section的title而已
 */
 ...
 */
 /*
 在section上加一个field
 add_settings_field()与do_settings_fields()
 */
 ...
 */
 /*
每种页面的参数会很多, 
wordpress支持的各页面的标准参数名放在$whitelist_options[$option_page]上,
但是你可以在这些页面上扩展几个section以支持更多的参数, 
一个页面可有多个section, 一个section可有多个field
section有自己的属性, field也有自己的属性, 如果用一个变量表示势必是
$section['title'] = ...
$section['fileds'] = [
		[],
		[]		
...
比较麻烦, 还不如
$section['title'] = ...
field['section_id']['title'] = 'field1 title'
?
 */
 ...
 */
 /*
显示某个$wp_settings_fields[$page][$section]上所有参数的输入画面
 */
