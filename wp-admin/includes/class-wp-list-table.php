...
http://wpengineer.com/category/wordpress-tutorials/page/2/
https://make.wordpress.org/docs/plugin-developer-handbook/10-plugin-components/custom-list-table-columns/  (讲得很好)
...
class WP_List_Table {
	...
	public function __construct( $args = array() ) {
		$args = wp_parse_args( $args, array(
			/*** 
			英语的单词有单数(singular)和复数(plural)2种不同写法, 对于汉语而言, 名词没有单复数之分 
			如'singular' => 'user', 'plural'   => 'users', 而对于汉语'singular' => '用户', 'plural'   => '用户',
			*/
			'plural' => '',
			'singular' => '',
			'ajax' => false,
		...
		/*** 
		可见通过apply_filters( "manage_{$screen->id}_columns", array() ); 可间接调用get_columns(), 这样做的好处是
		是可以动态增减要显示的字段
		*/
	 ...
	 */
	 /*** 
	 准备数据, 主要是:$this->items,  $this->_column_headers, $this->set_pagination_args()
	 */
...
<?php
	 ...
	 */
	 /*** 
	 向子类提供的接口
	 要想使用bulk_actions(), 子类必须定义 get_bulk_actions()
	 */	 
...
<?php
	 ...
	 */
	 /***
	 get_items_per_page('edit_post_per_page');	// 对于post页, 每页多少条记录
	 get_items_per_page('edit_page_per_page');	// 对于page页,每页多少条记录
	 ...
	 */
	protected function get_items_per_page( $option, $default = 20 ) {
		/*** 直接从wp_users_meta表中读取, 所以通过screen 调整后的参数, 这里也能读到 */
	 ...
	 * @param string $which
	 */
	 /*** 显示分页栏, 主要是在url中加paged参数, 如paged=123, $_GET['paged'] */
	 ...
	 */
	 /***
	在一张列表中, 如果想对某条记录执行删改动作, 那么
	记录中应当有一个id字段(即primary column), 所以在子类中
	定义一个get_default_primary_column_name()是有必要的	
	 */
	protected function get_default_primary_column_name() {
		...
		/*** 如果子类未提供get_default_primary_column_name(), 取第一个非checkbox的字段作为primary 字段 */
	 ...
	 */
	 /***
	 有时用户希望自己可以修改每页显示几行记录, 每行有哪些字段,其中有哪些属隐藏字段, 这就要通
	 过screen option来设置

	get_column_info()才是真正控制表头显示的, 
	
	 在prepare_items()中使用$this->_column_headers = $this->get_column_info(); 比直接调用get_columns()好, 
	 因为前者可读取到用户screen操作的设置?

	 如果在prepare_items()时,直接写死
        $columns = $this->get_columns();
	...
	protected function get_column_info() {
		...
		/*** get_column_headers()是通过fitler调用get_columns(), 当然用户有机会新增加自定义字段? */
		$columns = get_column_headers( $this->screen );
		
		/*** 
		columns是通过内存来配的, 而hidden_columns是需要保存到db中的, 
		get_hidden_columns() 会从wp_users_meta中读取 */
...
<table class="wp-list-table <?php echo implode( ' ', $this->get_table_classes() ); ?>">
	<thead>
	<tr>
		<?php $this->print_column_headers(); /* 表头*/ ?>
	</tr>
	</thead>
	...
	<tbody id="the-list"<?php
		...
		<?php $this->display_rows_or_placeholder(); /* 表体*/ ?>
	</tbody>

	<tfoot>
	<tr>
		<?php $this->print_column_headers( false ); /* 表头*/ ?>
	</tr>
	</tfoot>
