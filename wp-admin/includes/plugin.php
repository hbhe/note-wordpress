 ...
 */
 /*
获取plugin头注释中的Plugin Name, Plugin URI, ...
 */
function get_plugin_data( $plugin_file, $markup = true, $translate = true ) {

	$default_headers = array(
		// 在注释头中找'Plugin Name'串,将:后面的值 存在$plugin_data['Name']中
		'Name' => 'Plugin Name',
 ...
 */
 /* 系统支持的可自定义脚本即dropin , 可以理解成不能disable的插件*/
...
有时一个插件只是想设点参数, 没必要单独搞个一级菜单,可以将菜单项附在某个一级菜单下
用法如:add_options_page(), add_plugins_page(),...

有时甚至都不想建一个新页, 比如只想在已有的页上加几个字段能设置一下,可用
add_settings_section(), add_settings_field()
 */
function add_menu_page( $page_title, $menu_title, $capability, $menu_slug, $function = '', $icon_url = '', $position = null ) {
	global $menu, $admin_page_hooks, $_registered_pages, $_parent_pages;

	/* 保存到$menu, $admin_page_hooks中 */
	...
	if ( !empty( $function ) && !empty( $hookname ) && current_user_can( $capability ) )
		/* 
		将fuction挂到'toplevel_page_prowp_main_menu_slug'点上,何时执行? 
		见admin.php中的do_action( $page_hook );	
		在点击链接时,会do_action($hookname), 进而执行$function, 来显示页面的html片断
		*/
 ...
 */
 /***
在$submenu[$parent_slug]中注册一个菜单项
将$function挂到$menu_slug这个hook上

当首次add_menu_page()时, 是没有子菜单选项显示的
当首次add_submenu_page()时, 却会有2个子菜单选项显示出来
 */
function add_submenu_page( $parent_slug, $page_title, $menu_title, $capability, $menu_slug, $function = '' ) {
	 ...
	 */
	 /*** 添加子菜单时, 如果发现父菜单这个容器还是空的, 就先填充一个与父菜单功能一样的子菜单到容器中 */
 ...
 * @param string $parent_page
 */
 // 返回如'toplevel_page_prowp_main_menu_slug'
...
function register_setting( $option_group, $option_name, $sanitize_callback = '' ) {
	...
	/* $option_group页面可接受$option_name这个参数? */
 ...
 */
 /* 
 调整wordpress所支持的全部参数名(保存在$whitelist_options中),
 $new_whitelist_options为插件中新增的一些参数name
 将$whitelist_options merge到$new_whitelist_options中,然后返回$whitelist_options
 $new_whitelist_options是register_setting()中设置的
 */
