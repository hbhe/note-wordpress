...
function get_column_headers( $screen ) {
	...
	if ( ! isset( $column_headers[ $screen->id ] ) ) {
		 ...
		 */
		 /*** 当$screen->id = edit-post时, 就是apply_filters( "manage_edit-pos_columns"),
		 而WP_List_Table在构造函数中就会add_filter( "manage_{$this->screen->id}_columns", array( $this, 'get_columns' ), 0 );
		 因此实际上还是会调用list table对象中的get_columns()
