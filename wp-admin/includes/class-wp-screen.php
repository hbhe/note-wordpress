...
function cmi_add_option() { 
    $option = 'per_page'; // 改成别的串也可以,不过只有是'per_page'时,才显示在screen中, @see show_screen_options()
    $args = array(
        'label' => 'Movies',
        'default' => 10,
...
$option = $screen->get_option('per_page', 'option'); // 得到'array_sample_per_page'
...
final class WP_Screen {
	...
	public static function get( $hook_name = '' ) {
		...
		// For those pesky (麻烦的)meta boxes.
	 ...
	 */
	 /*** 大多数后台都会显示, 当前用户当前页面下的帮助 , screen option 表单*/
	...
	public function show_screen_options() {
		...
		/*** 如果全局$column_headers 变量中有东西, 或者通过add_screen_option( $option, $args )设置过'per_page' 这个option
		就要显示screen option栏目
		*/
