...
function populate_options() {
	...
	*/
	...
	'recently_edited' => '',
	'template' => $template,			/*** 父主题所在目录 */
	'stylesheet' => $stylesheet,			/*** 子主题(即style.css)所在目录 */
	'comment_whitelist' => 1,
	'blacklist_keys' => '',
...
function populate_roles_160() {
	...
	/*** 
	定义了5种角色,及每种角色的权限,
	新注册用户时,可以为其指定一种角色, 如下:
	$user_id = wp_create_user( $user_login, $pas1, $user_email );
	$user = new WP_User($user_id);
	$user->set_role('editor');	

	administrator: 单站内所有权限,包括网站设置
	editor:管理所有人的贴子
	author: 管理自己的贴子
	contributor: 管理自己的贴子(但不能发表)
	subscriber: 只能读贴子
	...
	*/
	...
	$role->add_cap('unfiltered_html');
	$role->add_cap('edit_posts');
	$role->add_cap('edit_others_posts'); /*** 编辑他人的贴子 */
	$role->add_cap('edit_published_posts');
	$role->add_cap('publish_posts');
	$role->add_cap('edit_pages');
	$role->add_cap('read');
