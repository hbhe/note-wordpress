...
<?php _e('Visibility:'); /* 显示'公开度' */ ?> <span id="post-visibility-display"><?php
...
echo esc_html( $visibility_trans ); /* 显示公开度的取值 */ ?></span>
<?php if ( $can_publish ) { ?>
...
<div class="misc-pub-section misc-pub-revisions">
	<?php printf( __( 'Revisions: %s' ), '<b>' . number_format_i18n( $args['args']['revisions_count'] ) . '</b>' ); /* 显示贴子的'版本' */ ?>
...
<div class="misc-pub-section curtime misc-pub-curtime">
	<span id="timestamp">
	<?php printf($stamp, $date); /* 显示发布时间 */  ?></span>
...
if ( current_user_can( "delete_post", $post->ID ) ) {
	...
	?>
<a class="submitdelete deletion" href="<?php echo get_delete_post_link($post->ID); ?>"><?php echo $delete_text; /* 移至回收站 */ ?></a><?php
} ?>
</div>
...
<?php
		...
		<input name="save" type="submit" class="button button-primary button-large" id="publish" value="<?php esc_attr_e( 'Update' ) /* 更新 */ ?>" />
<?php
} ?>
</div>
<div class="clear"></div>
...
function post_format_meta_box( $post, $box ) {
	/*** 
	如果支持'post-formats' 就显示
	是否支持post_format可在functions.php中设置 
	*/
	...
	<div id="post-formats-select">
		<fieldset>
			...
			<?php foreach ( $post_formats[0] as $format ) : /* 格式有:标准,日志,图像,...等 */ ?>
...
foreach ( $metadata as $key => $value ) {
	/*** 前面有_前缀的key是protected_meta, 就不要出现在输入界面了 */
...
<label class="screen-reader-text" for="parent_id"><?php _e('Parent')  /* 显示'父级'(页面属性中的) */ ?></label>
<?php echo $pages; /* 显示下拉列表 */ ?>
<?php
...
<p><strong><?php _e('Order') /* 显示'排序' */ ?></strong></p>
