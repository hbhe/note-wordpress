...
/* 
收集输入的$db_name信息, 读取wp-config-sample.php文件的内容, 用$db_name替换掉其中的DB_NAME等, 生成wp-config.php 
有了wp-config.php中的DB新常量, 下一步就可连接db并建表了
*/
...
<?php
	...
	/* $config_file中的内容来自wp-config-sample.php, 现在要替换其中的一些sample值, 如DB_NAME, DB_USER 的定义 */
...
<?php
		...
		/* 将替换后的$config_file内容生成wp-config.php文件, 有了这个文件,就可进行下一步install.php了 */
...
<p class="step"><a href="<?php echo $install; /* 如果点此链接,则执行下一步wp-admin/install.php */ ?>" class="button button-large"><?php _e( 'Run the install' ); ?></a></p>
<?php
	endif;
	break;
}
